...
There have been some syntax changes, and new settings, incorporated to ``manager.conf`` file. Please review this file manually in order to import your previous configuration. In addition, note note that ``agent.conf`` file directory has now changed to ``/var/ossec/etc/shared/default``.
