...
The Wazuh server comprises two main components; the :doc:`Wazuh manager <wazuh-manager>` and :ref:`Filebeat <indexer_integration_filebeat>`. The Wazuh manager is responsible for data analysis and alerting, while Filebeat forwards the analyzed data to the :ref:`Wazuh indexer <indexer_integration_wazuh_indexer>`. Refer to the :doc:`Wazuh server installation </installation-guide/wazuh-server/index>` documentation for information on how to install and set it up.
