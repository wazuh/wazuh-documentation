...
`PagerDuty <https://www.pagerduty.com/>`_ is a SaaS incident response platform suitable for IT departments. This integration allows to create a service using its official API in order to receive Wazuh alerts on the Incidents Dashboard. PagerDuty allows you to create a Developer Tools API and a Services API. For integration with Wazuh, you need to create a Services API. On PagerDuty select the Services menu, the Service Directory and then create a New Service selecting the integration type "Use our API directly" and the "Events API v2" option. Use the API code generated by the service to configure your PagerDuty integration.
