...
Log data is sent to the agent socket by default, but it is also possible to specify other sockets as output. ``wazuh-moduled`` uses UNIX type sockets to communicate allowing TCP or UDP protocols.
