...
The script is configured to run as part of the active response settings defined on the Wazuh manager, ``yara.sh``, needs to be placed under ``/var/ossec/active-response/bin`` on the Wazuh agent side. Add the following content to it:
...
.. note:: Make sure that you have `jq <https://stedolan.github.io/jq/>`_ installed, that the ``yara.sh`` file ownership is ``root:wazuh``, and the permissions are ``750``.
