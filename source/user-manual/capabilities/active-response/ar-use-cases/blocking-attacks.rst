...
Define the command in the ``manager.conf`` of your Wazuh manager:
...
Define the active response in the ``manager.conf`` of your Wazuh manager:
...
We can also set a list of IP addresses that should never be blocked by the active response. In global section of ``manager.conf`` in the Manager, use the field ``white_list``. It allows IP address or netblock.
