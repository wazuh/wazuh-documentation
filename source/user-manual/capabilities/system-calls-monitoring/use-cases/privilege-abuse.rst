...
In this use case, we have two users, John and Jane. We monitor Jane’s home directory and track whether any other user, not Jane or root, attempts to access it.
...
#. Edit the audit rule file ``/etc/audit/audit.rules`` and add the following configuration:
   ...
   -  ``-F arch=b64`` specifies the hardware CPU architecture of the monitored endpoint.
   -  ``-S open -S openat`` specifies that the rule applies when a process invokes the ``open`` or ``openat`` syscalls.
   ...
   -  ``-F auid!=-1`` specifies that the ``auid`` should not be equal to -1, which is used when a process is launched without a user ID.
   ...
   -  ``-F key=power_abuse`` provides a unique ID that Wazuh uses to analyze the audit logs.
