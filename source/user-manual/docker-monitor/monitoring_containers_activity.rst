...
The :doc:`Docker wodle <../reference/ossec-conf/wodle-docker>` collects events on Docker containers such as starting, stopping or pausing.
...
In order to use the Docker listener module it is only necessary to enable the ``wodle`` in the ``/var/ossec/etc/ossec.conf`` file of the server running docker, or this can also be done through :doc:`Centralized Configuration <../reference/centralized-configuration>`. It will start a new thread to listen to Docker events.
