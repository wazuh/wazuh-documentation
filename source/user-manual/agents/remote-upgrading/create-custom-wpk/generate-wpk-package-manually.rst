...
Modify the ``wazuh-3.12.0/etc/preloaded-vars.conf`` file that was downloaded to deploy an :ref:`unattended update <unattended-installation>` in the agent by uncommenting the following lines:
...
.. code-block:: console
  ...
  # curl -Ls https://packages.wazuh.com/3.x/windows/wazuh-agent-3.12.0-1.msi --output wazuh-agent-3.12.0-1.msi
