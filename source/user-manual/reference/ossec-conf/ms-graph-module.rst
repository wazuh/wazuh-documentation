...
.. meta::
  :description: Learn about the ms-graph configuration section of ossec.conf, which configures the collection of events from Microsoft Graph API resources.

.. _reference_ossec_ms_graph:
...
The ``<ms-graph>`` section configures the collection of events from Microsoft Graph API resources.
...
|                      | (bytes), k/K (kilobytes), m/M (megabytes), and g/G (gigabytes). Minimum value of 1M.         |
+----------------------+----------------------------------------------------------------------------------------------+
...
The name of the resource (i.e., specific API endpoint) to query for logs. Additional information on the Microsoft Graph REST API endpoints can be found at the `Microsoft Graph REST API v1.0 endpoint reference <https://learn.microsoft.com/en-us/graph/api/overview?view=graph-rest-1.0>`__.
