...
.. meta::
  :description: Learn about the wodle name="command" configuration section of ossec.conf, which configures scheduled command execution and output collection.

.. _reference_ossec_wodle_command:
...
Verify the MD5 sum of the binary or the script specified in the command option.
...
Verify the SHA1 sum of the binary or the script specified in the command option.
...
Verify the SHA256 sum of the binary or the script specified in the command option.
...
Run the defined command even when the checksum does not match. In this case, the agent logs that checksum verification failed but still runs the application.
...
Time of the day to run the scan. It must be represented in the format *hh:mm*.
...
.. note::
   When the time option is set, the interval value must be a multiple of days or weeks. By default, the interval is set to a day.
