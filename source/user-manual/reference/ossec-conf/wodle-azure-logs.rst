...
.. meta::
  :description: Learn about the wodle name="azure-logs" configuration section of ossec.conf, which configures the collection of logs from Azure Log Analytics, Microsoft Graph, and Azure Storage.

.. _reference_ossec_wodle_azure_logs:
...
For more information on Log Analytics query's language, check the `Azure documentation <https://docs.microsoft.com/en-us/azure/azure-monitor/logs/get-started-queries>`__.
...
+----------------------+----------------------------------------------------------------------------------------------+
| **Allowed values**   | A positive number that should contain a suffix character indicating a time unit, such as, m  |
...
Path of the file that contains the application identifier and the application key for authentication in order to use the Azure Active Directory Graph API. Incompatible with the ``application_id`` and ``application_key`` options.
...
+-------------------------------+----------------------------------------------------------------------------------------------+
| graph\\request\\query         | Any string containing auditLogs/directoryaudits, auditLogs/signIns or auditLogs/provisioning |
|                               | plus any optional query parameter                                                            |
+-------------------------------+----------------------------------------------------------------------------------------------+
...
- `storage\\container name`_
- `storage\\container\\blobs`_
- `storage\\container\\content_type`_
- `storage\\container\\timeout`_
- `storage\\container\\time_offset`_
- `storage\\container\\path`_
...
.. note::
   This option is related to the option ``content_type``, because if any blob has a different content to the one we have indicated, it will not be read correctly. Therefore, we need to be aware of what content we are trying to obtain and take it into consideration when using this option with "*".
