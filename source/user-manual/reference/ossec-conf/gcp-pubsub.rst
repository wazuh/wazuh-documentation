...
.. meta::
  :description: Learn about the gcp-pubsub configuration section of ossec.conf, which configures collection of logs from a Google Cloud Pub/Sub subscription.

.. _reference_ossec_gcp_pubsub:
...
.. note::
   The Wazuh GCP module supports only one gcp-pubsub section per Wazuh configuration file ``<ossec_conf>``. To configure more than one service, you need to deploy multiple agents.
