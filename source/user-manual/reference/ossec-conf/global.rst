...
This specifies an IP address for which Active Responses will not be triggered. Only one IP address may be specified for each ``<while_list>`` tag, but several IPs may be used by including multiple ``<white_list>`` tags.
