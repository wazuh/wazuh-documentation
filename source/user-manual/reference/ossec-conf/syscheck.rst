...
.. meta::
  :description: Learn about the syscheck configuration section of ossec.conf, which configures file integrity monitoring and Windows registry monitoring.
...
+----------------------+--------------------------------------------+
| **Allowed values**   | Any directory. Any environment variable.   |
+----------------------+--------------------------------------------+
...
``enabled`` — Specifies if the number of monitored entries has a limit.
...
``enabled`` — Set the disk quota limit option to enabled or disabled.
...
``enabled`` — Set the size limit of a file to enabled or disabled.
...
Specifies the who-data mode used by the FIM module. If the ``<provider>`` tag is not configured, the FIM module defaults to the audit mode. This option is only available for Linux endpoints.
