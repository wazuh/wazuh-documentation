...
.. topic:: Active response

  The active responses module manages the configured scripts. It also handles the socket needed to perform remote upgrades in the agents.

.. topic:: Log Collector

  The logcollector module monitors configured files and commands for new log messages.

  Logcollector module is now multi-threaded, achieving an improvement in overall performance. Each of the threads will read the first log that is not already handled by other threads
...
.. topic:: File Integrity Monitoring
  ...
  FIM module checks configured files for changes to the checksums, permissions and ownership.
