...
To generate and change passwords for all the Wazuh indexer users, run the script with the ``-a`` option:
  ...
  .. code-block:: console
    ...
    WARNING: Wazuh indexer passwords changed. Remember to update the password in the Wazuh dashboard, Wazuh server, Wazuh server keystore using wazuh-keystore tool, and Filebeat nodes if necessary, and restart the services.
