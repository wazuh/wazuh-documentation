...
#. Place the ``onelogin_metadata.xml`` file within the ``/etc/wazuh-indexer/opensearch-security/`` directory. Set the file ownership to ``wazuh-indexer`` using the following command:

   .. code-block:: console

      # chown wazuh-indexer:wazuh-indexer /etc/wazuh-indexer/opensearch-security/onelogin_metadata.xml
      ...
      ...
            ...
            saml_auth_domain2:
              ...
              http_authenticator:
                ...
                config:
                  idp:
                    metadata_file: '/etc/wazuh-indexer/opensearch-security/onelogin_metadata.xml'
...
#. Edit the ``/etc/wazuh-indexer/opensearch-security/roles_mapping.yml`` file and change the following values:
   ...
   To configure the ``roles_mapping.yml`` file, we map the ``Department`` field from step 3 to the ``all_access`` role on the Wazuh indexer. In this case, ``wazuh-admins``:
...
Follow these steps to integrate OneLogin  IdP with Wazuh for single sign-on and grant read-only role to the authenticated OneLogin users on the Wazuh platform:

#. :ref:`configuration_onelogin_ro`
#. :ref:`indexer_configuration_onelogin_ro`
#. :ref:`dashboard_configuration_onelogin_ro`

.. _configuration_onelogin_ro:
...
#. Create a new user.
   ...
   #. Select the user, navigate to **More Actions** and click on **Change Password** to assign a password to the user.
      ...
      .. thumbnail:: /images/single-sign-on/onelogin/03-click-on-save-user-RO.png
         :title: Click on Save User
         :align: center
         :width: 80%
...
.. _indexer_configuration_onelogin_ro:
...
.. _dashboard_configuration_onelogin_ro:
