...
#. (Optional) If you want to use the new index pattern by default, open the Wazuh Kibana Wazuh Kibana plugin configuration file (``/usr/share/kibana/optimize/wazuh/config/wazuh.yml``) and modify the ``pattern`` setting with the new one. It should be like this:
