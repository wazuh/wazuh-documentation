...
.. topic:: Pointing agents to the cluster nodes
    ...
    In this case, the agent 001 reports to the worker01-node node. To achieve this, we must modify the information contained in the labels ``<client><server>`` in the file ``/var/ossec/etc/agent.conf``, where we must place the IP address of the node we want to report to. In our case, it looks like this:
