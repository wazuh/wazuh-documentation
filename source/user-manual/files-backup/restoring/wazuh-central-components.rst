...
Wazuh, by default, compresses logs that are older than a day. While performing old log restoration in the `Restoring Wazuh server files`_ section, the old logs remain compressed.
