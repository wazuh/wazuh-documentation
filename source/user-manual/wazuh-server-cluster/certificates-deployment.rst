...
.. note::
   ...
   The admin certificate is specific to the Wazuh indexer cluster.  For more details, refer to the :doc:`documentation </user-manual/wazuh-indexer-cluster/certificate-deployment>`.
...
.. note::
   ...
   You need to use a pre-existing root CA to create Wazuh server certificates: If you already have a root CA, after generating certificates for the :doc:`Wazuh indexer </user-manual/wazuh-indexer-cluster/certificate-deployment>` or :doc:`Wazuh dashboard </user-manual/wazuh-dashboard/certificates>` nodes. If you need to reinstall a Wazuh server node or add a new node to your Wazuh server cluster.
