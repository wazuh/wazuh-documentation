...
The registration process consists of a mechanism to create a trust relationship between the Manager and an Agent, this process could be done in a Manager itself or with a registration service, this service it listen on the Manager, an Agent could request a pre-shared key using some credentials and it will reply with the pre-shared key and store the new Agent in local database.
