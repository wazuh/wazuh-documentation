...
The ``ossec-authd`` service is used to obtain a unique key, one per each Wazuh agent, which allows to authenticate with the Wazuh communication service and to encrypt traffic. The communication is done over TLS protocol.
...
Some registration methods automatically detect the IP of the Wazuh agent during the registration process.
