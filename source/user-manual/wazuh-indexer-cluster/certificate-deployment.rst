...
.. note::
   ...
   - If you already have a root CA generated during certificate creation for the :doc:`Wazuh server (all-in-one deployment) </user-manual/wazuh-server-cluster/adding-new-server-nodes/all-in-one-deployment#certificate-creation>` or :doc:`Wazuh server (distributed deployment) </user-manual/wazuh-server-cluster/adding-new-server-nodes/distributed-deployment#certificate-creation>` nodes, or for the :doc:`Wazuh dashboard </user-manual/wazuh-dashboard/certificates>` nodes.
