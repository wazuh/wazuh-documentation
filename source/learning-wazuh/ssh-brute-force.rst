...
Press Control+C to exit wazuh-logtest.  Then restart ``wazuh-logtest`` but this time with the ``-v`` parameter for more detail.

.. code-block:: console

    [root@wazuh-manager centos]# /var/ossec/bin/wazuh-logtest -v
...
The verbose output of ``wazuh-logtest`` is very helpful for understanding the hierarchical way in which rules are evaluated.
