...
Prior to the installation of the OSSEC rules for Amazon AWS, follow the steps below in order to enable logging through AWS API and to download the JSON data files. A detailed description of each of the steps can be found further below. 
...
5. Run the python script to download JSON data in gzipped files logs and convert it into a flat file.
...
Install AWS Cli in your Ossec Manager
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
Run a python script for download the JSON data
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
