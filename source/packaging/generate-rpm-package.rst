...
.. code-block:: console
  ...
  # ./generate_rpm_package.sh -b v3.11.1 -s /tmp -t manager -a x86_64 -r my_rev.

This will generate a 3.11.1 Wazuh manager RPM package with revision ``my_rev`` for ``x86_64`` systems.

.. code-block:: console

  # ./generate_rpm_package.sh -b v3.11.1 -s /tmp -t api -a i386 -r my_rev

This will generate a 3.11.1 Wazuh api RPM package with revision ``my_rev`` for ``i386`` systems and store it in ``/tmp``.

.. code-block:: console

  # ./generate_rpm_package.sh -b v3.11.1 -t agent -a x86_64 -p /opt

This will generate a 3.11.1 Wazuh agent RPM package with ``/opt`` as installation directory for ``x86_64`` systems.
