...
.. code-block:: console
  ...
  # ./generate_wpk_package.sh -t linux -b v3.11.2 -d /tmp/wpk -k /tmp/keys -o LinuxAgent.wpk

This will build a 3.11.2 Wazuh Linux WPK package named LinuxAgent.wpk, using the  with the previously generated keys that are saved in ``/tmp/keys`` and store it in ``/tmp/wpk``.
...
.. code-block:: console
  ...
  # curl -O https://packages.wazuh.com/3.x/windows/wazuh-agent-3.11.2-1.msi

Below, you will find an example of Windows WPK package building.

.. code-block:: console

  # ./generate_wpk_package.sh -t windows -b v3.11.2 -d /tmp/wpk -k /tmp/keys -o WindowsAgent.wpk -pd /tmp/wazuh-agent-3.11.2-1.msi

This will build a 3.11.2 Wazuh Windows WPK package named WindowsAgent.wpk, using the  with the previously generated keys that are saved in ``/tmp/keys`` and store it in ``/tmp/wpk``.
...
.. code-block:: console
  ...
  # ./generate_wpk_package.sh -t linux -b v3.11.2 -d /tmp/wpk -k /tmp/keys -o LinuxAgent.wpk -c /tmp/wpk_checksum
