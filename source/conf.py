...
linkcheck_ignore = [
    ...
    r'^https://DOMAIN/',
    # docutils auto-linkifies bare "http://"/"https://" scheme text inside format-illustration
    # prose (e.g. a table cell's allowed-value example showing "http://<address>:<port>"),
    # producing a host-less non-URL. Confirmed real instances:
    # user-manual/reference/wazuh-manager-conf/indexer.rst, user-manual/reference/ossec-conf/agent.rst.
    r'^https?:/$',
    r'^https://$',
    # A deliberately truncated, illustrative example value in a table cell (the source reads
    # "https://clients2.google.com/service/u..." — docutils' bare-URL autolink recognition
    # stops before the trailing dots, so the URI linkcheck actually extracts and reports is
    # ".../service/u" with no ellipsis at all) — never a real, complete hyperlink.
    # user-manual/capabilities/system-inventory/available-inventory-fields.rst.
    r'^https://clients2\.google\.com/service/u$',
    # Returns 403 to every request regardless of user-agent (confirmed via curl with a
    # browser UA) — CDN/bot-level blocking, not evidence the page itself is gone.
    r'^https://www\.npmjs\.com/?$',
]

# Client-side-rendered (SPA) doc sites with no server-rendered heading anchors at all —
# confirmed via a raw fetch of each (PagerDuty: only Gatsby framework IDs; Shuffler: only
# cookie-banner IDs; Azure portal: a client-side fragment-route, not a server anchor). Sphinx's
# linkcheck only parses static HTML, so these can never resolve even though a real browser
# renders them fine. Keep checking the base URL's reachability — just skip anchor
# verification for these domains.
linkcheck_anchors_ignore_for_url = [
    r'^https://developer\.pagerduty\.com/',
    r'^https://shuffler\.io/',
