...
In this section you will learn how to create a trail for your AWS account. Trails can be created using the AWS CloudTrail console or the AWS Command Line Interface (AWS CLI). Both methods follow the same steps:
...
S3 bucket name is common for all amazon users, don't worry if you get this error ``Bucket already exists. Select a different bucket name.``, even if you dont have any bucket before.

From now on all your actions in Amazon AWS will be logged. You can search logs manually inside ``CloudTrail/API activity history``. Also, notice that every 7 min a .json file will be saved in your bucket.
...
.. note:: User names can only use a combination of alphanumeric characters and these characters: plus (+), equal (=), comma (,), period (.), at (@), and hyphen (-). Names must be unique within an account. For more information about limitations on IAM entities
...
.. note:: If you installed a new version of Python alongside an older version that came with your distribution, or update pip to the latest version, you may get an error when trying to invoke pip with sudo and the result is ``command not found``. We can work around this issue by using ``which pip`` to locate the executable, and then invoke it directly by using an absolute path when installing the AWS CLI:
...
Where ``s3bucketname`` is the bucket name created when CloudTrail was activated and ``/var/log/amazon/amazon.log`` the path after convert the JSON file export the logs.
