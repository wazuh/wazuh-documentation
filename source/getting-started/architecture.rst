...
-  The Wazuh agent is deployed on endpoints to collect and forward security data to the Wazuh manager for data transformation and enrichment.
...
The Wazuh dashboard queries the indexed data from the Wazuh indexer to provide alerting and visualization. It also provides security and compliance dashboard, management capabilities for Wazuh agents, Wazuh manager configuration, health checks, security notifications, and alerting integrations.

Wazuh also supports agentless monitoring for systems and devices where installing the Wazuh agent is not possible. Network devices such as firewalls, switches, routers, and access points can actively forward log data via SSH.
...
The :doc:`Wazuh agent <components/wazuh-agent>` continuously sends events to the :doc:`Wazuh manager <components/wazuh-server>`. To start shipping this data, the Wazuh agent establishes a secure connection with the Wazuh manager service for agent connection, which listens on TCP port ``1514`` by default (this is configurable). The normalization engine in the Wazuh manager transforms and enriches the received events.
