...
Wazuh server uses the Wazuh forwarder to send alert and event data to the Wazuh indexer, using TLS encryption. Wazuh forwarder reads the Wazuh server output data and sends it to Wazuh indexer (by default listening on port 9200/TCP). Once the data is indexed by the Wazuh indexer, the Wazuh interface is used to mine and visualize the information.
