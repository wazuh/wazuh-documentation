...
If an instance has any open port which is receiving a brute force attack, the following alert will be shown on Wazuh dashboard. It shows information about the attacked host, about the attacker and which port is being attacked:
...
If an API call is made from an unsual network, the following alert will be shown on Wazuh dashboard. It shows the location of the unusual network, the user who did the API calls and which API calls it did:
...
If there is any indicator of a compromised EC2 instance, an alert will be shown on Wazuh dashboard explaining what's happening. Some example of alerts are shown below:
