...
.. code-block:: none
  ...
  Usage: ./generate_wazuh_splunk_app.sh [OPTIONS]
      ...
      -b, --branch <branch>     [Required] Select Git branch or tag e.g. v3.12.0-7.3.0
...
.. code-block:: console
  ...
  # ./generate_wazuh_app.sh -b v3.12.0-7.3.0 -s /splunk-app -r 1

This will generate a Wazuh Splunk app package for Wazuh 3.12.0 and Splunk 7.3.0 with revision 1 and store it in /splunk-app.

.. code-block:: console

  # ./generate_wazuh_app.sh -b v3.12.0-7.3.0 -s /wazuh-app -r 1 -c

This will generate a Wazuh Splunk app package for Wazuh 3.12.0 and Splunk 7.3.0 with revision 1, the sha512 checksum and store them in /splunk-app.
