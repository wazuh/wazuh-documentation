...
.. code-block:: console
  ...
  # ./generate_wazuh_packages.sh -b v3.12.0 -s /tmp

This will build a 3.12.0 Wazuh agent macOS  package and store it in ``/tmp``.

.. code-block:: console

  # ./generate_wazuh_packages.sh -b v3.12.0 -s /tmp -j 6

This will also build a 3.12.0 Wazuh agent macOS package and store it in ``/tmp`` but will use 6 jobs to compile the sources.

.. code-block:: console

  # ./generate_wazuh_packages.sh -b v3.12.0 -s /tmp -j 6 -c
...
.. code-block:: console
  ...
  $ sudo ./generate_wazuh_packages.sh -b v3.12.0 -j 4 -r 1 --notarize \
