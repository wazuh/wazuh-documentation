...
#.  The number of wazuh servers to be implemented will determine the next step. Choose ``Wazuh single-node cluster``, if there is only one Wazuh server, and ``Wazuh multi-node cluster`` in case there are two or more Wazuh servers.
