...
#. Copy or move the ``wazuh-install.sh``, ``wazuh-offline.tar.gz`` and ``wazuh-install-files.tar`` files to a directory accessible to the host(s) from where the offline installation will be carried out. This can be done by using ``scp``.
