...
There are two components that are usually installed on a Wazuh server: the manager and the API. In addition, for distributed architectures (where the Wazuh server sends data to a remote Elastic Stack cluster) you will need to install **Filebeat**.

There are several options for installing a Wazuh server depending on the operating system and whether or not you wish to build from source. Consult the table below to choose how to proceed:
