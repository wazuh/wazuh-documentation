...
^^^^^^^^^^^^^^^^^^^^^^
  ...
  #. Replace ``<server-node-name>`` with your Wazuh server node certificate name, the same as the one used in ``config.yml`` when creating the certificates. Then, move the certificates to their corresponding location.
