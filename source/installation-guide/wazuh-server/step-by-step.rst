...
Install and configure the Wazuh server as a single-node or multi-node cluster following step-by-step instructions. The Wazuh server is a central component that includes the Wazuh manager and Indexer Connector. The Wazuh manager collects and analyzes data from the deployed Wazuh agents. It triggers alerts when threats or anomalies are detected. The Indexer Connector securely forwards alerts and archived events to the Wazuh indexer.
...
Note that the IP addresses ``10.0.0.3``, ``10.0.0.4``, ``10.0.0.5`` are used as example.
