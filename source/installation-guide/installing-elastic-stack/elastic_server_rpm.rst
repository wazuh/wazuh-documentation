...
Logstash is the tool that will collect, parse, and forward data to Elasticsearch for indexing and storage of all logs generated by the Wazuh server. For more info please see `Logstash <https://www.elastic.co/products/logstash>`_.
...
3. **Optional.** Kibana will only listen on the loopback interface (localhost) by default. To set up Kibana to listen on all interfaces, edit the file ``/etc/kibana/kibana.yml`` uncomment the setting ``server.host``. Change the value to:
