...
This document will go through the installation of the Wazuh server components and the Splunk forwarder in a Wazuh single-node cluster.
...
The Wazuh server collects and analyzes data from the deployed Wazuh agents. It runs the Wazuh manager, the Wazuh API, and the Wazuh forwarder. The first step to set up Wazuh is adding the Wazuh's repository to the server, alternatively, all the available packages can be found :ref:`here <packages>`. 
