...
- Elastic Stack, including Open Distro for Elasticsearch as a single-node cluster or as a multi-node cluster, Filebeat and Open Distro for Kibana, including the Wazuh Kibana plugin, on the same host as Elasticsearch node or on a separate one.
