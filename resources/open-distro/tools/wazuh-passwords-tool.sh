...
getNetworkHost() {
    IP=$(grep -hr "network.host:" /etc/${SERVICE_NAME}/elasticsearch.yml)
    NH="network.host: "
    IP="${IP//$NH}"
}
...
checkInstalled() {
    ...
    if [ "${SYS_TYPE}" == "yum" ]; then
        elasticinstalled=$(yum list installed 2>/dev/null | grep wazuh-indexer)
    elif [ "${SYS_TYPE}" == "zypper" ]; then
        elasticinstalled=$(zypper packages --installed | grep wazuh-indexer)
    elif [ "${SYS_TYPE}" == "apt-get" ]; then
        elasticinstalled=$(apt list --installed  2>/dev/null | grep wazuh-indexer)
    fi 

    if [ -z "${elasticinstalled}" ]; then
        if [ "${SYS_TYPE}" == "yum" ]; then
            elasticinstalled=$(yum list installed 2>/dev/null | grep opendistroforelasticsearch | grep -v opendistroforelasticsearch-kibana)
        elif [ "${SYS_TYPE}" == "zypper" ]; then
            elasticinstalled=$(zypper packages --installed | grep opendistroforelasticsearch | grep i+ | grep noarch | grep -v opendistroforelasticsearch-kibana)
        elif [ "${SYS_TYPE}" == "apt-get" ]; then
            elasticinstalled=$(apt list --installed  2>/dev/null | grep opendistroforelasticsearch | grep -v opendistroforelasticsearch-kibana)
        fi
    else
        SERVICE_NAME="wazuh-indexer"
        echo "wazuh-indexer package is installed in the system"
    fi

    echo "${elasticinstalled}"
    if [ -z "${elasticinstalled}" ]; then
        echo "Error: Neither Open Distro nor wazuh-indexer are installed on the system."
        exit 1;
    else
        echo "wazuh-indexer package is installed in the system"
    fi
...
readUsers() {
    SUSERS=$(grep -B 1 hash: /usr/share/${SERVICE_NAME}/plugins/opendistro_security/securityconfig/internal_users.yml | grep -v hash: | grep -v "-" | awk '{ print substr( $0, 1, length($0)-1 ) }')
    USERS=($SUSERS)  
}
...
createBackUp() {
    ...
    eval "mkdir /usr/share/${SERVICE_NAME}/backup ${VERBOSE}"
    eval "cd /usr/share/${SERVICE_NAME}/plugins/opendistro_security/tools/ ${VERBOSE}"
    eval "./securityadmin.sh -backup /usr/share/${SERVICE_NAME}/backup -nhnv -cacert /etc/${SERVICE_NAME}/certs/root-ca.pem -cert /etc/${SERVICE_NAME}/certs/admin.pem -key /etc/${SERVICE_NAME}/certs/admin-key.pem -icl -h ${IP} ${VERBOSE}"
    if [  "$?" != 0  ]; then
...
generateHash() {
    ...
    if [ -n "${CHANGEALL}" ]; then
        ...
        for i in "${!PASSWORDS[@]}"
        do
            NHASH=$(bash /usr/share/${SERVICE_NAME}/plugins/opendistro_security/tools/hash.sh -p ${PASSWORDS[i]} | grep -v WARNING)
            HASHES+=(${NHASH})
        done
        echo "Hashes generated"
    else
        echo "Generating hash"
        HASH=$(bash /usr/share/${SERVICE_NAME}/plugins/opendistro_security/tools/hash.sh -p ${PASSWORD} | grep -v WARNING)
        if [  "$?" != 0  ]; then
...
changePassword() {
    ...
    if [ -n "${CHANGEALL}" ]; then
        for i in "${!PASSWORDS[@]}"
        do
           awk -v new=${HASHES[i]} 'prev=="'${USERS[i]}':"{sub(/\042.*/,""); $0=$0 new} {prev=$1} 1' /usr/share/${SERVICE_NAME}/backup/internal_users.yml > internal_users.yml_tmp && mv -f internal_users.yml_tmp /usr/share/${SERVICE_NAME}/backup/internal_users.yml
        ...
        awk -v new="$HASH" 'prev=="'${NUSER}':"{sub(/\042.*/,""); $0=$0 new} {prev=$1} 1' /usr/share/${SERVICE_NAME}/backup/internal_users.yml > internal_users.yml_tmp && mv -f internal_users.yml_tmp /usr/share/${SERVICE_NAME}/backup/internal_users.yml
...
runSecurityAdmin() {
    ...
    eval "rm -rf /usr/share/${SERVICE_NAME}/backup/ ${VERBOSE}"
    echo "Done"
...
main() {   
    ...
    if [ -n "$1" ]; then      
        ...
        export JAVA_HOME=/usr/share/${SERVICE_NAME}/jdk/
