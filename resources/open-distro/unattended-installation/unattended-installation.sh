...
installFilebeat() {
    ...
    if [  "$?" != 0  ]; then
        ...
        filebeatinstalled="1"
        eval "curl -so /etc/filebeat/filebeat.yml https://documentation.wazuh.com/4.1/resources/open-distro/filebeat/7.x/filebeat_unattended.yml --max-time 300  ${debug}"
...
installKibana() {
    ...
    if [  "$?" != 0  ]; then
        ...
        kibanainstalled="1"
        eval "curl -so /etc/kibana/kibana.yml https://documentation.wazuh.com/4.1/resources/open-distro/kibana/7.x/kibana_unattended.yml --max-time 300 ${debug}"
...
changePasswords() {
    eval "curl -so ~/wazuh-passwords-tool.sh https://documentation.wazuh.com/4.1/resources/open-distro/tools/wazuh-passwords-tool.sh --max-time 300 ${debug}"
    if [ -n "${verbose}" ]; then
