...
downloadPackages() {
    ...
    if [ ${sys_type} == "yum" ] || [ ${sys_type} == "zypper" ]; then    
        ...
        eval "curl -so ~/wazuh-packages/wazuh-manager_4.0.4-1_arm64.deb s3://packages.wazuh.com/4.x/apt/pool/main/w/wazuh-manager/wazuh-manager_4.0.4-1_arm64.deb --max-time 300 ${debug}"
        eval "curl -so ~/wazuh-packages/opendistroforelasticsearch_1.11.0-1_amd64.deb s3://packages.wazuh.com/4.x/apt/pool/main/o/opendistroforelasticsearch/opendistroforelasticsearch_1.11.0-1_amd64.deb --max-time 300 ${debug}"
        eval "curl -so ~/wazuh-packages/elasticsearch-oss-7.9.1-amd64.deb s3://packages.wazuh.com/4.x/apt/pool/main/e/elasticsearch-oss/elasticsearch-oss-7.9.1-amd64.deb --max-time 300 ${debug}"
