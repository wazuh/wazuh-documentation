...
installKibana() {
    ...
    if [  "$?" != 0  ]; then
        ...
        eval "sudo -u kibana /usr/share/kibana/bin/kibana-plugin install https://s3-us-west-1.amazonaws.com/packages-dev.wazuh.com/warehouse/test/4.0/ui/kibana/wazuh_kibana-4.0.0_7.8.0-0.0.0.todelete.zip ${debug}"
        if [  "$?" != 0  ]; then
