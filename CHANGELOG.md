...
- **Post-release**: Fixed a `console` block missing prompts on a multi-line command in the *Wazuh agent from sources* documentation, and a malformed `code-block` directive that silently dropped its content in the *Generate agent package* documentation. ([#10128](https://github.com/wazuh/wazuh-documentation/pull/10128))
