...
- Replaced the `admin` user with the dedicated `wazuh-manager` user in the Wazuh indexer connector credential examples, and standardized the indexer connector certificate name to `indexer-connector.pem`/`indexer-connector-key.pem` (previously inconsistent across pages) across the *Installation guide*, *Deployment options*, and *User manual* documentation, also correcting the certificate ownership and permissions in the *Installation guide*'s certificate deployment step so the Wazuh manager can read them after dropping privileges. ([#10063](https://github.com/wazuh/wazuh-documentation/pull/10063))
