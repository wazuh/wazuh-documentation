...
- Replaced the `admin` user with the dedicated `wazuh-manager` user in the Wazuh indexer connector credential examples across the *Installation guide*, *Deployment options*, and *Reference* documentation, and corrected the certificate names, ownership, and permissions in the *Installation guide*'s certificate deployment step so the Wazuh manager can read them after dropping privileges. ([#10063](https://github.com/wazuh/wazuh-documentation/pull/10063))
